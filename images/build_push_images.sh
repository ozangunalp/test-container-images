...
#####
# FOR EACH KAFKA VERSION BUILD IMAGE WITH DIFFERENT TAG (i.e., 'strimzi-test-container/test-container:0.1.0-kafka-kraft-2.8.1)
#####
for KRAFT_VERSION in $KRAFT_VERSIONS
do
    CURRENT_TAG="$PRODUCT_VERSION-kafka-kraft-$KAFKA_VERSION"
    echo "[INFO] Building images with following setup: DOCKER_VERSION_ARG=$DOCKER_VERSION_ARG, PROJECT_NAME=$PROJECT_NAME, PRODUCT_VERSION=$PRODUCT_VERSION, DOCKERFILE_DIR=$DOCKERFILE_DIR"

    for ARCH in $ARCHITECTURES
    do
        echo "[INFO] Building image with name: strimzi-test-container/$PROJECT_NAME:$CURRENT_TAG-$ARCH $KAFKA_VERSION with $SCALA_VERSION)."
        docker build --platform linux/$ARCH --build-arg version=$DOCKER_VERSION_ARG --build-arg KAFKA_VERSION=$KAFKA_VERSION --build-arg SCALA_VERSION=$SCALA_VERSION -t strimzi/$PROJECT_NAME:$CURRENT_TAG-$ARCH -f $DOCKERFILE_DIR/Dockerfile.kraft $DOCKERFILE_DIR
    done
done
